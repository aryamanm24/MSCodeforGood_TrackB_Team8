...
import { demoPantries } from "@/lib/data";

// The four pantries we show in the operator dropdown
const DEMO_NAMES = [
...
// Zip codes covering lower Manhattan where these pantries are located
...
// These are valid as long as the dataset hasn't materially changed.
...
/**
 * Map a raw Supabase JSONB resource object to the shape OperatorPanel expects.
...
export async function GET() {
  ...
  if (!supabase) {
    // Supabase not configured — return pre-computed static data
    return NextResponse.json({ source: "static", pantries: demoPantries });
  }

  try {
    // Fetch all published resources from lower Manhattan zip codes.
    // This is a small targeted query (~50-100 rows) rather than scanning all 388.
    ...
    // Produce the final array in DEMO_NAMES order.
    // Fall back to the static demoPantry for any pantry not found in Supabase.
    ...
    return NextResponse.json({ source: "supabase", pantries });
  } catch (err) {
